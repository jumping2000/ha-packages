...
**Ti piace questo package? Lascia una stella su Github e supportami per realizzarne altri!** <a href="https://www.buymeacoffee.com/jumping"><img src="https://cdn.buymeacoffee.com/buttons/default-yellow.png" height="20"></a><br>

<b> Blueprint per versione "lite" </b>:
...
2. Macchina a stati finiti _ESPHOME_ o _lite_ per ricavare lo stato della lavastoviglie;
...
<span style="color:red">**🚨 Come usuale prestate attenzione a collegare la vostra lavastoviglie ad un dispositivo smart che sia in grado di supportarne il carico, purtroppo gli incidenti domestici sono sempre dietro l'angolo. 🚨**</span>
...
Vediamo in dettaglio quali sono gli stati per la lavastoviglie, le transizioni da uno stato ad un altro sono guidate dalla potenza assorbita dalla lavastoviglie stessa e dal tempo di permanenza nello stato:

| Stato | Caratteristiche |
| :---: | --- |
| **Idle** | Stato iniziale a riposo della lavastoviglie | 
| **Lavaggio** | Avvio del programma di lavaggio con il riscaldamento dell'acqua |
| **Risciacquo** | Fase di risciacquo |
| **Asciugatura** | Stato finale di asciugatura delle stoviglie |

Questo stati sono calcolati nella versione FSM _full esphome_ del package e nella versione _lite_ rispettivamente da:
* FSM implementata nel codice ESPHome e restituita dal sensore Esphome `sensor.dishwasher_status`
* FSM realizzata con blueprint, restituisce il template sensor `sensor.dishwasher_status`

Ovviamente la versione FSM realizzata con ESPHOME sarà più precisa, _se opportunamente configurata con cura con i dati di potenza della propria lavastoviglie_, perché lo stato della lavastoviglie dipenderà da un'analisi più fine della potenza assorbita e degli stati. 

Anche per la versione _lite_ gli stati dell'automa sono quattro e identici alla versione FSM ESPhome, nulla vieta di personalizzare i nomi degli stati andando ad agire sull'apposita entità `input_select`.

<br>

Anche chi possiede una lavastoviglie smart che espone in HA lo stato dell'elettrodomestico potrà utilizzare il package aggiustando il codice in maniera minimale adattandolo ai dati esposti dall'integrazione smart, vediamo come nel seguente paragrafo.

---

Se l'utente possiede una lavastoviglie smart è possibile adattare il package e la card Lovelace: il lavoro non è immediato ma modificando opportunamente le principali entità con i dati provenienti dall'integrazione è fattibile.
...
| :---: | --- | --- | 
| sensor.**lavastoviglie**_operation_state  | stato operativo | sensor.dishwasher_status |
| sensor.**lavastoviglie**_remaining_program_time | tempo rimasto | sensor.dishwasher_time_left |
| switch.**lavastoviglie**_program_auto1 | programma 1 | da esporre nella card |
| switch.**lavastoviglie**_program_auto2 | programma 2 | da esporre nella card |
...
* Timer con vari programmi lavastoviglie per tenere sotto controllo il tempo trascorso
...
<table align="center">
  ...
  <tr>
      <td><div align=center><img width = "450" src="img/lavastoviglie_1.png"/></div></td>
      <td><div align=center><img width = 420 src="img/lavastoviglie_2.png"/></div></td>
  </tr>
</table>
...
* **sulla parte sinistra** è presente l'immagine grafica della lavastoviglie, i CSS presenti nella configurazione fanno in modo che ad ogni stato corrisponda una animazione.
* **nella parte centrale** è collocato il grafico della potenza assorbita dal lavastoviglie.
...
* **in basso** sotto l'immagine della lavastoviglie è presente la scelta del programma della lavastoviglie con il tempo rimasto.
...
* **lavastoviglie**: bottone che con il tocco singolo spegne/accende la presa collegata all'elettrodomestico, con il tocco prolungato apre la finestra con i dati e i grafici di energia recuperati dalle informazioni statistiche contenute nel DB di HA.
...
<table align="center">
    ...
    <tr>
        <td><div align=center><img width = 400 src="img/lavastoviglie_3.png"/></div></td>
        <td><div align=center><img width = 400 src="img/lavastoviglie_4.png"/></div></td>
    </tr>
	<tr>
	    <th><center>🌀 Lavaggio in corso 🌀</center></th>
...
Per chi usa la modalità [storage](https://www.home-assistant.io/dashboards/dashboards/) è presente un unico file (**card_lavastoviglie_storage.yaml**) che include la parte Lovelace.
...
|   │   │   ├── device_base.yaml
│   │   │   └── secrets.yaml
|   |   └── plug_lavastoviglie.yaml
│   │
│   ├── lovelace/
│   │   └── card_elettrodomestici/
│   │       ├── lavastoviglie_button_1.yaml
│   │       ├── lavastoviglie_button_5.yaml
|   |       ├── card_lavastoviglie_storage.yaml
│   │       └── card_lavastoviglie.yaml
|   |
│   ├── packages/
│   │   └── elettrodomestici/
│   │       ├── dishwasher_fsm.yaml
│   │       └── dishwasher_lite.yaml
│   │
│   ├── www/
│   │   └── hassiohelp/
│   │       └── pkg_elettrodomestici/
│   │           ├── lavastoviglie.png
│   │           ├── lavastoviglie_oblo.png
│   │           ├── lavastoviglie_panni.png
│   │           ├── sfondo_black.png
│   │           ├── sfondo_white.png
│   │           ├── sfondo_gray.png
│   │           └── sfondo_gray2.png
...
Nella sezione **"IMPOSTAZIONI DEL PACKAGE"** presente nella parte iniziale nel file _dishwasher_xxx.yaml_ sono da configurare una serie di parametri:
1. servizi di notifica;
2. i dati necessari per il funzionamento come lo switch della presa a cui è collegata la lavastoviglie o il sensore di potenza.
...
* _PLUG LAVASTOVIGLIE SWITCH: &CONF_SWITCH_ENTITY_ dove inserire l'entità _switch_ che comanda la presa smart;
* _PLUG LAVASTOVIGLIE POTENZA (W): &CONF_POWER_ENTITY_ dove inserire l'entità _sensor_ che espone la potenza;
* _PLUG LAVASTOVIGLIE ENERGIA (kWh): &CONF_ENERGY_ENTITY_ dove inserire l'entità _sensor_ che espone l'energia;
...
* "_Cicli di avviso manutenzione_" indica dopo quanti cicli il sistema manda una notifica per effettuare operazioni di manutenzione (esempio lavaggio interno).
...
* _PLUG_LAVASTOVIGLIE_XXX_: associazione con le entità relative a vari parametri come potenza, energia etc.
...
* `sensor.dishwasher_status`: sensore che riporta lo stato in cui si trova la lavastoviglie;
* `sensor.dishwasher_time_left`: l'entità contiene il tempo alla fine del programma di lavaggio;
* `sensor.PUN_f1`: sensore scraping che restituisce il costo dell'energia PUN in EUR/kWh nella fascia F1;
* `binary_sensor.dishwasher_status`: sensore che riporta lo stato on/off;
* `binary_sensor.dishwasher_maintenance`: indica che è venuto il momento di fare una pulizia approfondita 😜;
* `input_number.lavastoviglie_ripetizioni_notifica`: numero di ripetizioni della notifiche a fine lavaggio;
* `input_number.lavastoviglie_avviso_manutenzione`: numero di cicli di manutenzione dopo i quali **HA** invia una notifica di avviso;
* `input_number.costo_energia`: costo dell'energia in EUR/kWh configurato dall'urgente se non si vuole il sensore scraping;
* `automation.lavastoviglie_status`: è la principale automazione del package "FSM full" che notifica gli eventi di cambio di stato;
* `automation.avviso_manutenzione_lavastoviglie`: notifica l'avvertimento per fare le operazioni di manutenzione programmata;
* `automation.lavastoviglie_program`: è l'automazione che permette il cambio di programma;
...
stateDiagram-v2
  direction LR
  [*] --> Idle
  Idle --> Prelavaggio: POWER_MEDIUM
  Lavaggio --> Risciacquo: POWER_MEDIUM
  Prelavaggio --> Lavaggio: POWER_HIGH
...
* una raffinata modellazione dei consumi elettrici della lavastoviglie (ESPhome FSM)
